...
export const FeaturesTab = ({ project }: FeaturesTabProps) => {
  ...
  const saveToDatabase = async (nodesToSave = nodes, edgesToSave = edges) => {
    if (!project?.id) {
      console.error('â?No project ID available for saving features');
      return;
    }
    ...
    try {
      ...
      console.error('â?Failed to save features:', error);
      throw error;
    } finally {
      setIsSaving(false);
    }
  ...
  const handleDeleteNode = useCallback(async (event: React.MouseEvent, nodeId: string) => {
    ...
    if (!project?.id) {
      console.error('â?No project ID available for deleting node');
      return;
    }
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      console.error('â?Failed to delete node:', error);
